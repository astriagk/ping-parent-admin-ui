...
export class OfferSmProductSliderComponent {
  @ViewChild('trendingProductSliderContainer')
  trendingProductSliderContainer!: ElementRef;
  @ViewChild('onSellSliderContainer') onSellSliderContainer!: ElementRef;
  @ViewChild('topRatedSellSliderContainer')
  topRatedSellSliderContainer!: ElementRef;
  ...
  constructor(private productService: ProductService) {
    this.productService.products.subscribe((products) => {
      this.trending_slider_products = this.chunkArray(
        products.filter((p) => p.trending),
        3
      );
      this.discount_slider_products = this.chunkArray(
        products.filter((p) => p.discount! > 0),
        3
      );
      this.top_rated_slider_products = this.chunkArray(
